...
def extend_dim(x: Tensor, dim: int):
    ...
    return x.view(*x.shape + (1,) * (dim - x.ndim)) #改变ｘ的形状
...
class VDiffusion(Diffusion):
    def __init__(
        ...
        super().__init__()
        self.net = net
        self.sigma_distribution = sigma_distribution #获得一个０－１之间随机noise schedule
    ...
    def forward(self, x: Tensor, **kwargs) -> Tensor:  # type: ignore
        ...
        sigmas = self.sigma_distribution(num_samples=batch_size, device=device) #sigmas=tensor([0.8903], device='cuda:0') sigmas是随机的
        sigmas_batch = extend_dim(sigmas, dim=x.ndim) #改变ｘ的形状  sigmas_batch = torch.Size([1, 1, 1]) tensor([[[0.8903]]], device='cuda:0')
        # Get noise
        noise = torch.randn_like(x)  #noise.shape = torch.Size([1, 1, 262144])     x.shape = torch.Size([1, 1, 262144])
        # Combine input and noise weighted by half-circle
        alphas, betas = self.get_alpha_beta(sigmas_batch)
        x_noisy = alphas * x + betas * noise     # 当前的 xt  即加噪声之后的ｘｔ
        v_target = alphas * noise - betas * x  #真实的ｖ
        # Predict velocity and return loss
        v_pred = self.net(x_noisy, sigmas, **kwargs) #预测的ｖ
        return F.mse_loss(v_pred, v_target)         #此处是mse损失 可以换成多层感知的音乐损失 减少计算量
