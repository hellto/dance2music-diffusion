...
#             t2 = torch.zeros(batch_size, 64, 768 - text.size()[2], device='cuda:1')
...
#              # text_embedding = embedder(text)  # type: ignore
#
#             t2 = torch.zeros(batch_size, 64, 768 - text.size()[2], device='cuda:0')
#             text= torch.cat((text, t2), 2)
#
#             # text_embedding = text.squeeze(0) 删除第０个维度
#             text_embedding = text
...
class motion_encoder(nn.Module):
    def __init__(self):
        ...
        model = [
            nn.Conv1d(120, 256, kernel_size=6), #输入的通道数为１２０，维度是２１９
            nn.ReLU(),
                ]
...
class vqEncoder_high(nn.Module): #感觉这个网络提取信息时丢失了很多时序信息
    ...
    def forward(self, x, genre, batch_size):
        x = x.float()  # x.Size([2,1 ,3072]) torch.float32   genre.Size([2, 10])torch.float32
        genre_idx = genre.nonzero(as_tuple=True)[1]  #genre_idx = tensor([2, 3], device='cuda:0') torch.int64
        genre_emb = self.genre_embed(genre_idx)  #genre_emb = torch.Size([2, 256])  torch.float32
        genre_emb = genre_emb.unsqueeze(1)  #genre_emb = torch.Size([2,1 ,256]) torch.float32
