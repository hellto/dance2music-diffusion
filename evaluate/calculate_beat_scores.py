...
flags.DEFINE_string(
    'audio_dir', '../Generate/train_con_66tensorboard/test4/',
    'Path to the AIST wav files.')
flags.DEFINE_string(
    'audio_cache_dir', '../Generate/train_con_66tensorboard/test10_aist_audio_feats_fullname/',
...
def motion_peak_onehot(joints):
    ...
    envelope = np.sum(velocity, axis=1)  # (seq_len,) #envelope(356,)

    # Find local minima in velocity -- beats
    peak_idxs = scisignal.argrelextrema(envelope[:356], np.less, axis=0, order=10) # 10 for 60FPS
...
#####################################################################
    ...
    # seq_names = []
    # 设定文件路径
    path = FLAGS.audio_cache_dir
    # 获取该目录下所有文件，存入列表中
    fileList = os.listdir(path)

    motion_path = '/home/tina/data/audio-diffusion-pytorch-main/edge_aistpp/smpl_motion_219/test_data/'
    n_samples = len(fileList)
    beat_scores = []
    for i,filename in enumerate(fileList):
        logging.info("processing %d / %d" % (i + 1, n_samples))
        motion = np.load(os.path.join(motion_path,filename))
        ...
        audio_feature = np.load(os.path.join(FLAGS.audio_cache_dir, filename))
        audio_beats = audio_feature[:motion_beats.shape[0], -1]  # last dim is the music beats
