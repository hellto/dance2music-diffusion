...
flags.DEFINE_string(
    'audio_dir', '../Generate/train_con_66tensorboard/test10/',
    'Path to the AIST wav files.')
flags.DEFINE_string(
    'audio_cache_dir', '../Generate/train_con_66tensorboard/test10_aist_audio_feats/',
...
def cache_audio_features(seq_names_fulls):
    ...
    for audio_name in audio_names:
        ...
        if os.path.exists(save_path):
            continue
        data, _ = librosa.load(os.path.join(FLAGS.audio_dir, f"{seq_names_fulls[i]}"), sr=SR)
        i=i+1
